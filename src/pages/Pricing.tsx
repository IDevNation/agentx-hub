const plans = [
  {
    ...
    amount: "$0",
    sub: "",
    desc: "Try any agent. No card needed.",
  ...
  },
  {
    ...
    amount: "$0.01–0.10",
    sub: "/use",
    desc: "Pay exactly for what you use.",
  ...
  },
  {
    ...
    amount: "$29",
    sub: "/mo",
    desc: "For power users. Lock in lower rates.",
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-12">
          {plans.map((p) => (
            <div key={p.tier} className={`bg-card border rounded-2xl p-8 relative transition-transform hover:-translate-y-1 ${p.featured ? "border-primary bg-gradient-to-br from-primary/5 to-card" : "border-border"}`}>
              ...
              <button className={`w-full px-4 py-3 rounded-lg text-sm font-medium transition-all ${p.featured ? "bg-primary text-primary-foreground hover:opacity-90" : "border border-border text-foreground hover:border-primary hover:text-primary"}`}>
                {p.cta}
              </button>
            </div>
          ))}
